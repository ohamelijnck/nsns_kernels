...
def get_config(t='all'):
    ...
    for ls_i, ls in enumerate(lengthscales):
        for i in range(1):
            config_arr.append(
                {
                    'file_prefix': 'split_gp_sep_st_{ls_i}_{i}'.format(i=i, ls_i=ls_i),
    ...
    for ls_i, ls in enumerate(lengthscales):
        for i in range(9):
            config_arr.append(
                {
                    'file_prefix': 'random_gp_sep_st_0_{i}_{ls_i}'.format(i=i, ls_i=ls_i),
...
def main(CONFIG, return_m=False, force_restore=False):
    ...
    k = gpflow.kernels.RBF(2, lengthscales=CONFIG['lengthscale'],ARD = True)
